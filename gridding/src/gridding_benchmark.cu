...
#define KERNEL_GRIDDER_TEMPLATE(current_nr_channels)                                                                   \
    ...
    __global__ void kernel_gridder(
        const int grid_size, int subgrid_size, float image_size, float w_step_in_lambda,
        int nr_channels, // channel_offset? for the macro?
        int nr_stations, UVWCoordinate<float> *uvw,
        float *wavenumbers, float2 *visibilities, float *spheroidal, float2 *aterms, Metadata *metadata,
        float2 *subgrids)
    {
        int channel_offset = 0;
        KERNEL_GRIDDER_TEMPLATE(8);
...
kernel_gridder_v8_(const int grid_size, int subgrid_size, float image_size,
                   ...
                   float2 *aterms, Metadata *metadata, float2 *subgrids) {
  int tidx = threadIdx.x;
  int tidy = threadIdx.y;
  int tid = tidx + tidy * blockDim.x;
  int nr_threads = blockDim.x * blockDim.y;
...
kernel_gridder(const int grid_size, int subgrid_size, float image_size,
  ...
  KERNEL_GRIDDER_TEMPLATE(1);
        ...
        Array2D<float> &spheroidal, Array4D<std::complex<float>> &subgrids, Array1D<Metadata> &metadata)
    {
        ...
        for (size_t i = 0; i < configuration.niterations; ++i)
        {
            //            kernel_gridder_v1<<<n_subgrids, NUM_THREADS>>>(
            //                configuration.grid_size, configuration.subgrid_size, image_size, w_step_in_lambda,
            //                configuration.nchannels, configuration.nstations, d_uvw, d_wavenumbers, d_visibilities,
            //                d_spheroidal, d_aterms, d_metadata, d_subgrids);

            kernel_gridder<<<n_subgrids, NUM_THREADS>>>(configuration.grid_size, configuration.subgrid_size, image_size,
