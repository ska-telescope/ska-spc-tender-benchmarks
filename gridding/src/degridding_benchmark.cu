...
__device__ void prepare_shared(const int current_nr_pixels,
                               const int pixel_offset,
                               const int nr_polarizations, const int grid_size,
                               const int subgrid_size, const float image_size,
                               const int nr_stations, const int aterm_idx,
                               const Metadata &metadata,
                               const float *spheroidal, const float2 *aterms,
  ...
  for (int j = tid; j < current_nr_pixels; j += num_threads) {
    ...
    // Apply aterm
    int station1_idx =
        index_aterm(subgrid_size, nr_stations, aterm_idx, station1, y, x);
    int station2_idx =
        index_aterm(subgrid_size, nr_stations, aterm_idx, station2, y, x);
    ...
    shared_v6[0][j] =
        make_float4(pixel[0].x, pixel[0].y, pixel[1].x, pixel[1].y);
    shared_v6[1][j] =
        make_float4(pixel[2].x, pixel[2].y, pixel[3].x, pixel[3].y);
...
__device__ void compute_visibility(const int nr_polarizations,
                                   const int current_nr_pixels,
                                   const int channel, const float u,
...
__global__ void kernel_degridder_v6(int grid_size, int subgrid_size,
                                    float image_size, float w_step_in_lambda,
                                    int nr_channels, int nr_stations,
                                    UVWCoordinate<float> *uvw,
                                    float *wavenumbers, float2 *visibilities,
                                    float *spheroidal, float2 *aterms,
                                    Metadata *metadata, float2 *subgrids) {
  ...
  for (int pixel_offset = 0; pixel_offset < nr_pixels;
    ...
    for (int time = 0; time < nr_timesteps; time += NUM_THREADS) {
      ...
      // Prepare data
      prepare_shared(current_nr_pixels, pixel_offset, 4, grid_size,
                     subgrid_size, image_size, nr_stations, aterm_index, m,
                     spheroidal, aterms, subgrids);
      ...
      for (int i = tid; i < ALIGN(NUM_THREADS * nr_channels, NUM_THREADS);
           i += NUM_THREADS) {
        ...
        // Compute visibility
        compute_visibility(4, current_nr_pixels, channel, u, v, w, wavenumbers,
                           visibility);
...
template <>
benchmark_result
degridding_benchmark_launcher<benchmarks_common::hardware_type::gpu>::launch(
    const gridding_benchmark::configuration &configuration,
    Array2D<UVWCoordinate<float>> &uvw,
    Array3D<Visibility<std::complex<float>>> &visibilities,
    Array1D<Baseline> &baselines,
  ...
  const auto d_visibilities_size =
      visibilities.size() * sizeof(visibilities[0]);
  ...
  cudaCheck(cudaMemcpy(d_wavenumbers, wavenumbers.data(), d_wavenumbers_size,
                       cudaMemcpyHostToDevice));
  cudaCheck(cudaMemcpy(d_visibilities, visibilities.data(), d_visibilities_size,
                       cudaMemcpyHostToDevice));
  cudaCheck(cudaMemcpy(d_spheroidal, spheroidal.data(), d_spheroidal_size,
                       cudaMemcpyHostToDevice));
  cudaCheck(cudaMemcpy(d_aterms, aterms.data(), d_aterms_size,
                       cudaMemcpyHostToDevice));
  cudaCheck(cudaMemcpy(d_metadata, metadata.data(), d_metadata_size,
                       cudaMemcpyHostToDevice));
  ...
  float in_copy_time;
  cudaCheck(
      cudaEventElapsedTime(&in_copy_time, begin_in_memcpy, end_in_memcpy));

  const auto n_baselines =
      (configuration.nstations * (configuration.nstations - 1)) / 2;
  ...
  kernel_degridder_v6<<<n_subgrids, NUM_THREADS>>>(
      configuration.grid_size, configuration.subgrid_size, image_size,
      w_step_in_lambda, configuration.nchannels, configuration.nstations, d_uvw,
      d_wavenumbers, d_visibilities, d_spheroidal, d_aterms, d_metadata,
      d_subgrids);
  ...
  cudaCheck(cudaMemcpy(d_subgrids, subgrids.data(), d_subgrids_size,
                       cudaMemcpyHostToDevice));
  cudaCheck(cudaEventRecord(end_out_memcpy));

  float out_copy_time;
  cudaCheck(
      cudaEventElapsedTime(&out_copy_time, begin_out_memcpy, end_out_memcpy));

  benchmark_result result;
  result.in_transfer_time = in_copy_time;
  result.out_transfer_time = out_copy_time;
  result.in_bandwidth =
      static_cast<float>(total_in_size) / (1000.f * in_copy_time);
  result.out_bandwidth =
      static_cast<float>(total_out_size) / (1000.f * out_copy_time);
